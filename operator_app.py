...
from job_matcher import analyze_with_ai
...
def render_job_pipeline_tab():
    """Tab 2: show locally stored jobs. Discovery from job sites comes later."""
    st.info(
    ...
    labels = [f"{job.get('company', '(no company)')} — {job.get('title', '(no title)')}" for job in jobs]
    selected = st.selectbox("Job", range(len(jobs)), format_func=lambda index: labels[index])
    current_status = jobs[selected].get("status", "Discovered")
    ...
    if st.button("Update Status"):
        jobs[selected]["status"] = new_status
        save_jobs(jobs)
